...
5. [cib_top.v：架构的"大脑"](#5-cib_topv架构的大脑)
   - 5.6 [时序与管线：ext_rdy 如何工作](#56-时序与管线ext_rdy-如何工作)
...
### 5.6 时序与管线：ext_rdy 如何工作

**核心问题**：外部总线和内部寄存器逻辑之间需不需要加寄存器（打拍）？

回答这个问题之前，先理解 `ext_rdy` 是怎么工作的：

```
没有管线的情况（当前 cib_top 的默认行为）：
...
**问题场景**：当外部总线时钟频率很高（比如 >100MHz），或者外部协议本身需要串行转并行（eSPI、SMBus），组合逻辑路径可能太长，导致时序违例。这时候就需要**打拍器（Pipeline Register）**来切断组合路径。

**谁来决定是否需要打拍？**

不是 cib_top 决定，而是**外部总线的特性**决定的：

| 场景 | 需要打拍？ | 原因 |
|------|-----------|------|
| 外部协议已有寄存输出（如同步并行总线） | 通常不需要 | 地址/数据已经寄存过了 |
| 外部协议是串行转并行（eSPI、SMBus、I2C） | ✅ 推荐 1 拍 | 解码输出的组合路径太长 |
| 芯片布局导致走线延迟很大 | ✅ 推荐 1~2 拍 | 物理时序需要 |
| 外部时钟频率 > 200MHz | ✅ 推荐 1 拍 | 留余量 |

**如何利用 ext_rdy 支持管线？**

`ext_rdy` 信号就是为了让外部主机知道**数据什么时候有效**。打拍后，rdy 跟着延迟，外部主机自然会等待：
...
读操作：
  时钟    █▁█▁█▁█▁█▁█▁█▁█▁█▁█
  addr   ────┃  0x1C00  ┃────────────  ← 协议包装器发出地址
                                          （打拍寄存 1 拍）
                                    ┌─ 这里是组合译码+读数据
  rdata  ──────────────────┃ 0xABCD ┃──  ← 数据延迟 1 拍到达
  rdy    ──────────────────┃   1   ┃──  ← rdy 也延迟 1 拍
  
  外部主机看到 rdy=1 才知道数据有效，所以时序仍然正确。
```

也就是说：**ext_rdy 天然支持管线延迟**，你只需要在协议包装器里控制 rdy 的时序，不需要改 cib_top 的任何逻辑。

**重要提醒**：

如果加了管线，`ext_rdy` 响应会比地址晚 N 拍。外部主机的超时计数器要留够余量。对于单次寄存器访问，1~2 拍延迟没有任何功能影响。但如果是**大批量连续读写**，延迟会影响吞吐率，这时可以考虑增加 burst 支持（但那是更高级的话题了，本文不展开）。
...
**问题回顾**：外部总线的协议解码输出可能需要打拍才能满足时序，这个打拍器放在哪一层？

#### 方案 A：独立一层 `cib_pipe.v`

```
外部协议 ──→ cib_top_xxxbus.v ──→ cib_pipe.v ──→ cib_top
```

多了一层 hierarchy，打拍逻辑和协议解码拆到两个文件里。**不推荐**，原因：
- 多了一层无谓的模块，代码量没有减少
- 管线深度是协议相关的属性，分到两个文件反而难维护
- 协议包装器和管线寄存器的接口定义需要额外对齐

#### 方案 B：放在 `cib_top_xxxbus.v` 内部（✅ 推荐）
...
cib_top_espi.v 内部：
    ...
    │
    ├── 段二：管线寄存（时序逻辑）
    │      原始 ext_* 值 → 寄存 1~N 拍 → 接 cib_top
    │
    └── cib_top 例化
```

**推荐理由**：
- 协议解码和打拍是"同一个问题的两个步骤"，放一起更内聚
- 管线深度根据协议特性自由调整，不影响其他模块
- 其他 cib_xxx 模块完全感知不到管线的存在

#### 方案 C：放在 `cib_top.v` 内部

```
cib_top 内部加寄存器
```

**不推荐**。cib_top 是通用模块，不该知道外部总线的时序特性。而且 cib_top 被所有项目共用，加了打拍器会增加所有项目的不必要延迟。

#### 方案 B 的代码模板

完整的示例已经在 `cib/cib_top_espi.v` 里，下面是核心模板：

```verilog
module cib_top_xxxbus (
    // 总线引脚...
);

    // ── 段一：协议解码（组合逻辑）──
    wire        xxx_cs_active_pre;
    wire        xxx_oe_pre;
    wire        xxx_we_pre;
    wire [15:0] xxx_addr_pre;
    wire [15:0] xxx_wdata_pre;
    // ... 把外部协议信号译成这些值 ...
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            ext_we_r    <= 1'b0;
            ext_addr_r  <= 16'h0000;
            ext_wdata_r <= 16'h0000;
        end else begin
            ext_cs_n_r  <= ~xxx_cs_active_pre;   // 取反
            ext_oe_r    <= xxx_oe_pre;
            ext_we_r    <= xxx_we_pre;
            ext_addr_r  <= xxx_addr_pre;
            ext_wdata_r <= xxx_wdata_pre;
        end
    end
    ...
    // ── 回读数据寄存（可选，对齐输出时序）──
    reg [15:0] espi_rdata_r;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            espi_rdata_r <= 16'h0000;
        else if (cib_rdy)
            espi_rdata_r <= cib_rdata;
    end

endmodule
```

**移植到新项目时**：复制这个模板，替换段一的协议解码逻辑，调整管线级数（1 级不够就再加一级 reg），其他不动。
...
│                                   - 移植新协议时不改这个文件
│
├── cib_top_espi.v                ← eSPI 协议包装器示例（新项目移植的模板）
│                                   - 协议解码 + 管线寄存 + cib_top 例化
│                                   - 换协议时复制此文件改协议解码部分
│                                   - 详细说明见第 9.4 节
│
