...
```verilog
// 在地址 0x0015~0x001F 放 11 个保留寄存器
reg_rsvd #(.W(16)) u_rsvd_1 ();
reg_rsvd #(.W(16)) u_rsvd_2 ();
// ...更多
```

**甚至可以不给端口连线**——因为 `reg_rsvd` 只有一个输出 `rdata`，而它的输出是固定 0。
...
always @(*) begin
    case (addr)
        REG_A : rdata_mux = reg_a_rdata;
        REG_B : rdata_mux = reg_b_rdata;
        // 中间有地址空洞……
...
用 default 也可以，但 `reg_rsvd` 的好处是：
- 明确标注了哪些地址是保留的
- 将来在这些地址上添加新寄存器时，把 `reg_rsvd` 替换成真正的寄存器即可
...
```
    ...
    ───────────────► │  reg_w1c           │  ← 写1清零
                     ...
                     └────────────────────┘
                              ...
                              ▼
                    ┌────────────────────┐
                    │  reg_base.v        │  ← 版本信息 (用了 reg_rw)
                    │  reg_status.v      │  ← 状态监控 (全 RO)
                    │  reg_int.v         │  ← 中断控制 (用了 reg_rw×3)
                    │  reg_spi.v         │  ← SPI 控制 (用了 reg_pulse)
                    └────────────────────┘
```
...
1. **在 reg_base.v 添加一个新的 RW 寄存器**（地址 0x0030）：
   ```verilog
   reg_rw #(.W(8), .INIT(8'h42)) u_my_reg (
       .clk(clk), .rst_n(rst_n),
       .load(write_active && (addr == 16'h0030)),
       .wdata(wdata[7:0]),
       .rdata(my_reg_rdata)
   );
   ```
   然后在 read mux 里加一行 `16'h0030 : rdata_mux = my_reg_rdata;`
